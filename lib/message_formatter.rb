    ...
    if links.length > 0
      ...
      if link =~ /(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/
        begin
          ...
          formatted += "</div><div class='video-embed'><div class='video-wrapper'><iframe width='350' height='240' frameborder='0' class='autoembed' allowfullscreen src='http://youtube.com/embed/#{code}' /></div></div>"
        rescue
        end
      end
    end
