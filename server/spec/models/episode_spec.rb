...
RSpec.describe Episode, type: :model do
  subject { create(:episode, anime: anime) }
  ...
  context 'triggers recalculation of episode length on anime' do
    it 'after destroy' do
      episode = create(:episode, anime: anime)
      expect(anime).to receive(:recalculate_episode_length!)
      episode.destroy!
    end
    it 'when length changed' do
      episode = create(:episode, anime: anime, length: rand(10..40))
      expect(anime).to receive(:recalculate_episode_length!)
  ...
  it 'should default length to the anime episode length' do
    ...
    episode = create(:episode, anime: anime, length: nil)
    expect(episode.length).to eq(length)
  end
  describe '::length_mode' do
    before do
      10.times { create(:episode, anime: anime, length: 10) }
      2.times { create(:episode, anime: anime, length: 20) }
    end
  ...
  describe '::length_average' do
    before do
      10.times { create(:episode, anime: anime, length: 10) }
      10.times { create(:episode, anime: anime, length: 20) }
    end
